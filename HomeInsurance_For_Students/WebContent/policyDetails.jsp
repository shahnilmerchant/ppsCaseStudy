...
function submitPage(policyKey, action, policyEndDate, policyStatus)
{
    var currdate = new Date();
    endDate = new Date(policyEndDate);
    oneMonthPrior=new Date(policyEndDate);
    oneMonthPrior.setMonth(oneMonthPrior.getMonth() - 1);
    var flag=0;
    
    if((action == "Renew" && (policyStatus == "ACTIVE" || policyStatus == "RENEWED") && currdate>oneMonthPrior && currdate <endDate)
            ||(action == "Renew" && policyStatus == "CANCELLED") || (action == "Cancel")) {
        ...
        alert("Policy cannot be renewed");
    }
}
</Script>
</head>
...
<h4 align="center">Welcome
    ...
    <table bgcolor="yellow" border="1">
        ...
        <tr>
            ...
            {
            %>
            <td><img src="images/arrow1.jpg" onclick="submitPage('<%=policy.getPolicyKey()%>', 'Renew', '<%=policy.getPolicyEndDate()%>','<%=policy.getPolicyStatus()%>')"/></td>
            <td><img src="images/arrow1.jpg" onclick="submitPage('<%=policy.getPolicyKey()%>', 'Cancel', '<%=policy.getPolicyEndDate()%>','<%=policy.getPolicyStatus()%>')"/></td>
            <%
            }
            %>   
        </tr>
